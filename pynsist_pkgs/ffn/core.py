...
class PerformanceStats(object):
    ...
    def _calculate(self, obj):
        ...
        self.weekly_mean = np.nan
        self.weekly_vol = np.nan
        self.weekly_sharpe = np.nan
        self.weekly_sortino = np.nan
        self.best_week = np.nan
        self.worst_week = np.nan
        ...
        # W-Sun = week with Sunday last day
        self.weekly_prices = obj.resample('W-Sun').last()
        ...
        if type(self.rf) is float:
            self.daily_sharpe = r.calc_sharpe(rf=self.rf, nperiods=252, annualize=False)
            self.daily_sortino = calc_sortino_ratio(r, rf=self.rf, nperiods=252, annualize=False)
        ...
        # stats using weekly data
        self.weekly_returns = self.weekly_prices.to_returns()
        wr = self.weekly_returns

        if len(wr) < 2:
            return

        self.weekly_mean = wr.mean() * 52
        self.weekly_vol = np.std(wr, ddof=1) * np.sqrt(52)

        if type(self.rf) is float:
            self.weekly_sharpe = wr.calc_sharpe(rf=self.rf, nperiods=52, annualize=False)
            self.weekly_sortino = wr.calc_sortino_ratio(rf=self.rf, nperiods=52, annualize=False)
        # rf is a price series
        else:
            _rates = self.rf.resample('W-Sun').last()
            self.weekly_sharpe = wr.calc_sharpe(rf=_rates, nperiods=52, annualize=False)
            self.weekly_sortino = wr.calc_sortino_ratio(rf=_rates, nperiods=52, annualize=False)
        self.best_week = wr.max()
        self.worst_week = wr.min()

        # -2 because p[-1] will be mp[-1]
        self.wtd = dp[-1] / wp[-2] - 1

        # -1 here to account for first return that will be nan
        self.pos_week_perc = len(wr[wr > 0]) / float(len(wr) - 1)
        self.avg_up_week = wr[wr > 0].mean()
        self.avg_down_week = wr[wr <= 0].mean()
        ...
        for idx in wr.index:
            if idx.year not in self.return_table:
                self.return_table[idx.year] = {1: 0, 2: 0, 3: 0,
                                               ...
                                               7: 0, 8: 0, 9: 0,
                                               10: 0, 11: 0, 12: 0}
            if not np.isnan(wr[idx]):
                self.return_table[idx.year][idx.week] = wr[idx]
        # add first month
        fidx = wr.index[0]
        try:
            self.return_table[fidx.year][fidx.week] = float(wp[0]) / dp[0] - 1
        except ZeroDivisionError:
            self.return_table[fidx.year][fidx.week] = 0

        if len(wr) < 3:
            return

        denom = dp[:dp.index[-1] - pd.DateOffset(weeks=3)]
        if len(denom) > 0:
            self.three_week = dp[-1] / denom[-1] - 1

        if len(wr) < 4:
            return

        self.weekly_skew = wr.skew()

        # if all zero/nan kurt fails division by zero
        self.weekly_kurt = wr.kurt() if len(wr[(~np.isnan(wr)) & (wr != 0)]) > 0 else 0.0

        denom = dp[:dp.index[-1] - pd.DateOffset(weeks=6)]
        if len(denom) > 0:
            self.six_week = dp[-1] / denom[-1] - 1
        ...
        if type(self.rf) is float:
            self.monthly_sharpe = mr.calc_sharpe(rf=self.rf, nperiods=12, annualize=False)
            self.monthly_sortino = mr.calc_sortino_ratio(rf=self.rf, nperiods=12, annualize=False)
        # rf is a price series
        else:
            _rates = self.rf.resample('M').last()
            self.monthly_sharpe = mr.calc_sharpe(rf=_rates, nperiods=12, annualize=False)
            self.monthly_sortino = calc_sortino_ratio(mr, rf=_rates, nperiods=12, annualize=False)
        ...
        if type(self.rf) is float:
            if self.yearly_vol > 0:
                self.yearly_sharpe = yr.calc_sharpe(rf=self.rf, nperiods=1, annualize=False)
            self.yearly_sortino = yr.calc_sortino_ratio(rf=self.rf, nperiods=1, annualize=False)
        # rf is a price series
        else:
            _rates = self.rf.resample('A').last()
            if self.yearly_vol > 0:
                self.yearly_sharpe = yr.calc_sharpe(rf=_rates, nperiods=1, annualize=False)
            self.yearly_sortino = yr.calc_sortino_ratio(rf=_rates, nperiods=1, annualize=False)
...
def calc_sharpe(returns, rf=0., nperiods=None, annualize=True):
    ...
    if isinstance(rf, pd.Series):
        ret = returns * nperiods
        er = np.mean(ret - rf)
    else:
        years = np.unique(returns.dropna().index.year)
        if len(years) > 1:
            ret = np.power(np.prod(returns + 1), 1 / len(years)) - 1 # annualized return
        else:
            ret = returns.sum() * nperiods / len(returns) 
        er = ret - rf
    
    std = np.std(returns) * np.sqrt(nperiods) # annualized std
    res = np.divide(er, std)
...
def calc_sortino_ratio(returns, rf=0., nperiods=None, annualize=False):
    """
    ...
    std = np.std(negative_returns, ddof=1) * np.sqrt(nperiods)
    res = np.divide(er, std)
