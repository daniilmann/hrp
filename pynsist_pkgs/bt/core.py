    ...
    def allocate(self, amount, update=True):
        ...
        if not q == -self._position:
            ...
            while not np.isclose(full_outlay, amount, rtol=0.) and q != 0:
                ...
                if self.integer_positions:
                    q = math.floor(q - np.max((1, int(dq_wout_considering_tx_costs))))
                else:
                ...
                if self.integer_positions:
                    ...
                    full_outlay_of_2_more, _, _ = self.outlay(q + 2)

                    if full_outlay < amount and (full_outlay_of_1_more > amount or full_outlay_of_2_more > amount):
                        break
